...
server = Flask('')
@server.route('/')
def home(): return "KURUT INFINITY ULTRA v25 | PO OTC ACTIVE"
...
# --- [4] ULTRA ACCURACY MATH ENGINE ---
def get_pocket_option_analysis():
    # Симуляция мощнейшего анализа (RSI, Bollinger, MACD, Stochastic + Trend)
    accuracy = random.uniform(91.2, 98.8)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПРОВЕРКА ДОСТУПА: Если уже VIP, кидаем сразу в меню
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "v_guide":
        ...
        text = (
            ...
            "2. Пополни баланс от $15 (Standart) или $35 (VIP).\n"
            f"3. Твой ID: `{uid}` — отправь его {ADMIN_USER}\n\n"
            "После проверки админ откроет тебе вечный доступ."
        )
    ...
    if query.data == "m_top":
        ...
        text = "🏆 **ТОП 10 ТРЕЙДЕРОВ KURUT:**\n━━━━━━━━━━━━━━\n"
        if not sorted_top: text += "Список пока пуст. Будь первым!"
        ...
        msg = await query.edit_message_text(f"📡 **ГЛУБОКИЙ МАТЕМАТИЧЕСКИЙ АНАЛИЗ...**\n`Инструмент: {asset}`")
        await asyncio.sleep(5)
        
        dir, acc, log = get_pocket_option_analysis()
        res = (
            f"✨ **ШИКАРНЫЙ СИГНАЛ ГОТОВ!**\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📊 АКТИВ: `{asset}`\n"
            f"⏳ ВРЕМЯ: `{tf}`\n"
            f"🚦 ВХОД: **{dir}**\n"
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('wait_bal'):
        try:
            ...
            text = f"🏃 **ПЛАН МАРАФОНА НА 30 ДНЕЙ ($ {bal})**\n━━━━━━━━━━━━━━\n"
            curr = bal
            for d in range(1, 31):
                p = round(curr * 0.15, 2)
                text += f"**День {d}:** `${round(curr, 2)}` ➡️ `${round(curr + p, 2)}`\n"
                curr += p
                if d == 15: text += "--- СЕРЕДИНА ---\n"
            text += f"\n🏆 **ИТОГ: `${round(curr, 2)}`**"
    ...
    if uid in [str(a) for a in ADMIN_IDS]:
        ...
        if txt.startswith("/grant"):
            ...
            try:
                await context.bot.send_message(chat_id=tid, text="🎉 **АДМИНИСТРАТОР ОТКРЫЛ ВАМ ВЕЧНЫЙ ДОСТУП!**\nТеперь тебе доступен весь функционал бота.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📊 ПОЛУЧИТЬ СИГНАЛЫ", callback_data="m_home")]]))
            except: pass
        elif txt.startswith("/revoke"):
            tid = txt.split()[1]
            if tid in vip_users: vip_users.remove(tid); save_db(DB_VIP, list(vip_users))
            await update.message.reply_text(f"❌ Доступ закрыт для `{tid}`")
...
if __name__ == "__main__":
    ...
    print("🚀 KURUT INFINITY v25 STARTED")
    app.run_polling()
